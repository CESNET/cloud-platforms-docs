...
For advanced VM monitoring, the QEMU guest agent provides guest-level metrics. You can query these through `virtctl guest-agent` commands or view them in Grafana when the agent is installed inside the VM. See [Using virtctl](./using-virtctl) for details.
