...
| `p3.*` | compute HPC, top performance on ephemeral storage         | No  | Yes, not guaranteed <sup>1</sup> | No | Yes <sup>2</sup> | ~ PASSMARK 2000, 1:1 | `hpc.*ephem` |
| `a3.*` | compute HPC, top performance on ephemeral storage         | No  | No  | Yes      | Yes <sup>2</sup>               | ~ PASSMARK 2000, 1:1 | `hpc.*gpu` |
...
<sup>2</sup> *The ephemeral disk presence may depend on instance setup. See [here](/docs/iaas-cloud/how-to-guides/instances-with-ephemeral-disk) for details.*
